 ...
 */
function isTopicLoaded(topic) {
  const stmt = db.prepare('SELECT COUNT(*) as count FROM questions WHERE topic = ?');
...
function loadTopicQuestions(topic) {
  ...
  console.log(`[Questions] Parsing questions for topic: ${topic}`);
 ...
 */
function ensureTopicLoaded(topic) {
 ...
 */
function getRandomUnansweredQuestion(topic) {
 ...
 */
function getRandomFailedQuestion(topic) {
  const stmt = db.prepare(`
